...
sections concern themselves with
\emph{type-checking}, i.e., with verifying that the knowledge base
satisfies  typing and cardinality constraints and with finding out what
causes their violation.
The primitives described in those sections must be invoked by
...
constraints: i.e., generating timely warnings for the user when a violation occurs in the cause of an
update transaction and, if possible, rolling back the changes made by the violating
...
happen---are more useful, since such queries allow us to find the
``culprits''
...
%% 
\begin{verbatim}
   ?-  +constraint{COI(?,?)}.  // activate the COI constraint
\end{verbatim}
%% 
The plus sign says that we would like to \emph{activate} the aforesaid query as an
integrity constraint; the minus sign is used when we want to
\emph{deactivate}  a constraint:
\index{constraint\{...\}!deleting}
%% 
\begin{verbatim}
   ?-  -constraint{COI(?,?)}.  // deactivate the COI constraint
\end{verbatim}
%% 
The first argument of \texttt{constraint\{...\}} must be a HiLog predicate
or a frame. It can also be a \texttt{\bs{}neg} of a predicate or a frame, but
...
often questionable.  The practical approach offered by \ERGO is to use a separate
Studio environment endowed with support for explanations and then 
...
fact that John consults for Acme was supposed to be
deleted right after he was hired by Acme as an employee).
If, on the other hand, it is \emph{known} how to resolve the violation of
...
\end{verbatim}
%% 
Suppose that next we execute the transaction that uses onlly backtrackable
inserts and deletes:
%% 
\begin{verbatim}
...
\end{verbatim}
%% 
If this insertion is performed, the knowledge base would get a
...
also appear, but the insertion would remain in the database  and the user would
need to remove the leftover garbage manually afterwards.
...
Sometimes, the knowledge engineer might need to take a programmable action
in response to a constraint violation. In most of the
previous examples, constraint
violations were just reported to the user and manual intervention was needed
to take a corrective action. To let the user specify programmable
...
\end{alltt}
%% 
The callback predicate must be a HiLog predicate formula with one
to four
arguments; it can be also a \texttt{\bs{}neg} of such a predicate. 
Since it is a HiLog predicate, it can actually be quite complex, e.g.,
\texttt{mycallback(foo,?X)(?Y,?Z)(?Arg1,...,?Arg4)}.
The callback must assume that it will be invoked  by \ERGO with
the  first argument bound to a list of the violating instances of our
constraint, so it should be an unbound variable in the call.
The other three arguments are supposed to help the user find out where the
violation took place. Specifically, the second argument to the callback is
expected
to be the file in which the constraint activation
command lives, the third argument is assumed to be the line number of that
command, and the fourth argument must be the module name where the file was
loaded or added.
These parameters can be easily obtained if we use the following
quasi-constants provided by the \ERGO compiler: \verb|\@F|, for the file;
\verb|\@L|, for the line; and \verb|\@| for the module.
If some callback does not need some of these parameters,
they can be omitted in certain cases. Concretely, the following
combinations  are possible: 
%% 
\begin{verbatim}
    mycallback(?ListOfViolatingInstances)
    mycallback(?ListOfViolatingInstances, \@F)
    mycallback(?ListOfViolatingInstances, \@F,\@L)
    mycallback(?ListOfViolatingInstances, \@F,\@L,\@)
\end{verbatim}
%% 
The arguments 2--4 can thus be used to help the user debug or verify the
correctness of an application.
Since \texttt{mycallback} can have a rather complex form itself (like
\texttt{cb(foo,?X)(?Y,?Z)}), the call to a callback might have variables
other than \texttt{?Arg},..., \texttt{?Arg4}  (like \texttt{?X},
\texttt{?Y}, \texttt{?Z}, which can be used to pass additional information
to and from the callback.    


If a callback is provided, then constraint-violating alerts won't be
printed  by \ERGO. Instead, \textit{CallbackPredicate} is called when
a violation is detected. The first argument in that call is
bound to the list of
violating instances of the constraint, and the callback could take a
