...
\end{verbatim}
%% 
will output the following (sans the top line, which is added by us for readability):
%% 
\begin{verbatim}
...
This part of the 
API is \textbf{for expert users only} whose \textbf{applications require
  talking}    to
the underlying \textbf{XSB engine directly}.   
Skip this section if you are not yet comfortable with the higher-level
\FLSYSTEM interface.

To access XSB from Python, use the
the commands  \texttt{pyxsb\_command()} and \texttt{pyxsb\_query()}.
The first is like
...
\item  \texttt{ERGOVariable(name=}\emph{string}\texttt{)}: this type of
  ...
  almost always different from what was in the user's  query. The only thing that
...
\end{itemize}
%% 
Typically, unpacking of an answer takes the form of testing what kind of
