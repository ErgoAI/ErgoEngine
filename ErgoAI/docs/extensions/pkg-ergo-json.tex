...
structures that contain comments like in C, Java, etc. The multiline
...
The same can actually be obtained much easier, using the
...
\item  \texttt{?Src[parse2memory(?Mod)]@\bs{}json}:
  ...
  JSON structure, which must be a JSON object or a list of JSON objects
  and inserts the corresponding facts, as explained above, into the \ERGO module
  \texttt{?Mod}.
