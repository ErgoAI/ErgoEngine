...
%% 
\begin{verbatim}
    ?-  +alert{conflict,employee(?,?,?), employee_alert_callback(?,\@F)}.
\end{verbatim}
%% 
Recall that \texttt{\bs{}@F} above is a quasi-constant denoting the current
module. 

The syntax of a callback in a conflict alert
is the  and itame as in the case of constraints: it must be a HiLog predicate defined
via some set of rules and it must have 1 to 4 arguments with the 
...
\begin{verbatim}
    ...
    \neg longtime_or_current_employee(?Company,?Empl) :-
            ...
            ?Year2 >= ?Year,
            ?Year2 - ?Year < 4.
    ?-  +alert{conflict,longtime_or_current_employee(?,\@F)}.
\end{verbatim}
%% 
...
%% 
\begin{verbatim}
    +alert{truth,longtime_or_current_employee(?,\@F)}.
\end{verbatim}
%% 
