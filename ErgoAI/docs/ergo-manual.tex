...
\newcommand{\ERGO}{\mbox{\ensuremath{\cal{E}}\smaller{{\sc{RGO}}}}\xspace}
%%\newcommand{\ERGOAI}{\mbox{\smaller{\ensuremath{\cal{E}}\smaller{{\sc{RGO}}}}\ensuremath{\cal{AI}}}\xspace}
\newcommand{\ERGOAI}{\mbox{{\ensuremath{\cal{E}}\smaller{{\sc{RGO}}}}\ensuremath{\cal{AI}}}\xspace}
...
is a sophisticated multi-paradigm  knowledge representation and
...
\item[] \textbf{Linux:} Ubuntu, Mint, Fedora, SUSE, etc., are likely to be
  ...
  \item A possible Java issue on Linux.  If your installation of the Java JDK
    ...
    \item The program \texttt{makedepend} often must be installed in advance.
      In Ubuntu, it resides in the package \texttt{xutils-dev}, which must
      be installed. You can find the package where \texttt{makedepend}
      belongs using
      %% 
\begin{verbatim}
...
\end{verbatim}
      %% 
      if you have an APT-based Linux. On an RPM-based Linux, use
      %% 
\begin{verbatim}
     yum whatprovides makedepend
\end{verbatim}
      %% 
  \item  One may also want to install these \textbf{optional} items:
    \begin{itemize}
    \item MySQL or MariaDB (\textbf{cannot} have both
      because these DBs use some of
      the same resources, like sockets), UnixODBC and MyODBC developer's packages. These are required
      ...
      unixodbc-dev, mysql-server, mysql-client,
      libmysqld-dev,
      or similar.
      (Alternatively, for MariaDB, use libmariadb-dev-compat, libmariadb-dev,
      mariadb-server).
      Use your system's package manager to find out if these packages are
      installed (and the packages' exact names) and install them if not.
    \item The package pcre-devel, libpcre-devel, libpcre3-dev, libpcre++-dev, or a similarly
    ...
    \item The package rlwrap is useful for the overall experience with the
      ...
      keyboard.
    \end{itemize}
    How would you know the exact names of the above developer packages on
    your system? Fortunately, every major Linux distribution has a search
    command. For APT-based Linuxes, one can use 
    %% 
    \begin{quote}
    ...
    \end{quote}
    %% 
    Here is partial output of the above command on Ubuntu 24:
    %% 
\begin{verbatim}
v   libcurl-dev          
v   libcurl-ssl-dev     
...
\end{verbatim}
    %% 
    RPM-based Linuxes, such as CentOS, use  
    %% 
    \begin{quote}
      \texttt{
      yum list regular-expression
      }
    \end{quote}
    %% 
  \end{itemize}
...
\item[] \textbf{Windows:} 
  ...
  \item Download the file \texttt{ergoAI\_\textnormal{\emph{versn}}.exe}
    (e.g., \verb|ergoAI_3.1.exe|) from Github, where
...
By default, \FLSYSTEM prints out all answers (if their number is
finite). If only one answer at a time is
desired, then type the following command: {\tt \bs{}one}.
In this case, \FLSYSTEM returns the first answer and then waits for user
input. To get the
next answer, type ``\texttt{;}'' immediately followed by \texttt{<ENTER>}. 
The user can continue typing \texttt{;<ENTER>} as long as more answers are
needed or until no more answers are left. If the query has additional
answers, but the user does not need to see them then he can just hit
\texttt{<ENTER>} and the interactive display of the answers will stop and
one will see the \FLSYSTEM prompt.  
One can revert back to
...
\begin{alltt}
 ...
 ?X = 'ErgoAI 3.1 (Arete; build: , 2024-07-11) on x86_64-pc-linux-gnu'
...
\begin{example}
  ...
  a fragment of an \FLSYSTEM knowledge base that represents a database of scientific
  publications.
\end{example}
...
\tt ++~ -\,-~ \&\& & list append/difference/intersection & right & binary \\ \cline{1-4}
\hline
\end{tabular}
}
...
Typed variables are associated with classes, which restricts such variables so that
...
%% 
\begin{verbatim}
    likes(?X^^Person,?Y^^Book) :- hasRead(?X,?Y,?Times^^\long), ?Times>1.
...
By default, \ERGO treats type expressions like
\verb|^^(A,(B-C) ; \dateTime)| above as abstract symbolic expressions that
have no relationship to the symbols used in these expressions.
However, the  runtime directive 
%% 
\begin{verbatim}
 ?- setsemantics{class_expressions=on}.
\end{verbatim}
%% 
makes \ERGO to actually evaluate those expressions treating the operands
as sets of objects of the named classes (like \texttt{A}, \texttt{B},
\texttt{\bs{}dateTime}).   
This option can also be turned off:
%% 
\begin{verbatim}
 ?- setsemantics{class_expressions=off}.
\end{verbatim}
%% 

Theoretically \verb|?X^^Cl| is equivalent to performing the test
\texttt{?X:Cl} \emph{whenever} \texttt{?X} \emph{gets bound by another subgoal},
...
\texttt{Cl}, which can be expensive if the class is large and may not
terminate, if the class is infinite (e.g., \texttt{?X:\bs{}integer}). 
In contrast, \verb|?X^^Cl| does not involve any tests until \texttt{?X}
gets bound. This is convenient because oftentimes the knowledge engineer does
not know which subgoal will eventually bind \texttt{?X} and so the guessing game
of where to put the test \texttt{?X:Cl} is avoided.
However, the knowledge engineer \emph{must assure}    that \texttt{?X}
\emph{will} become bound at some point or else no checks will be performed whatsoever
(because \verb|?X^^Cl| causes the membership test for \texttt{Cl} to be
performed only when \texttt{?X} gets bound to a non-variable).    
To avoid these problems, it would be handy if \verb|?X^^Cl| were always
evaluated to \verb|?X^^Cl|, if \texttt{Cl} is an infinite class. In
general, determining if a class defined by rules is infinite is undecidable.
For standard \emph{primitive} data types, described in
Section~\ref{sec-data-types}, we know that apart from \verb|\boolean| these
types are infinite so \ERGO evaluates such queries in the
described way. For example,  we get
%% 
\begin{verbatim}
  ?- ?x:\integer.
  ?x = ?_h11006^^(\long)
\end{verbatim}
%% 
because \verb|\integer| is an infinite class and its name is standardized
to \verb|\long|. In contrast, the subgoal \verb|?x:\boolean|
is evaluated by successively binding \texttt{?x} to \verb|\true|,
\verb|\false|, and \verb|\undefined|. 
In the future, we might let the user to ``advise'' \ERGO which user-defined
classes can be assumed to be infinite thereby making the above technique more
widely available.
...
\end{verbatim}
%% 
The \texttt{setsemantics} directive tells the system that 
it should start treating expressions like \texttt{(A,B)}, \texttt{(A;B)}
or \texttt{(A-B)} as operations on the sets of member-objects in the named
classes (\texttt{A}, \texttt{B}, ...).   
...
\end{verbatim}
%%
is true because for all \texttt{?X}'s that belong to class \texttt{foo} and
...
\item \texttt{isnafform\{Term\}} --- true if \texttt{Term} is of the form
  ...
  \index{iscallable\{...\} primitive}
\index{primitive!iscallable\{...\}}
\item \texttt{iscallable\{Term\}} --- true if \texttt{Term} is a
  truth-valued \FLSYSTEM statement.

  \index{istransactional\{...\} primitive}
\index{primitive!istransactional\{...\}}
...
\index{issysloaded\{Module\} primitive}
\index{primitive!issysloaded\{...\}}
...
\subsubsection{Importing \FLSYSTEM Predicates into Prolog Programs}

To import an \FLSYSTEM predicate into a Prolog shell, the following must be done:
%%
%%
\begin{itemize}
\item First, the command
...
\end{quote}
%%
must be executed in Prolog. If you are importing \FLSYSTEM predicates into a
...
the following additional directive  must appear near the top of {\tt test.P}, \emph{prior to any call} to
  \FLSYSTEM predicates:
   %%
  \begin{verbatim}
    :- import ('\\ergoimport')/1 from flora2.
  \end{verbatim}
  %%
\item 
  \index{'\bs{}\bs{}ergoimport' operator}
  %%
  One of the following {\tt '\bs{}\bs{}ergoimport'} commands must be executed in the shell:
  \begin{quote}
    \tt
        ?- '\bs{}\bs{}ergoimport' {\it ergo-predicate/arity} as {\it
      xsb-name}(\_,\_,...,\_)\\
  ...
  \end{quote}
%%
We will explain shortly which {\tt '\bs{}\bs{}ergoimport'} command should be
used in what situation.
Note that a double-backslash is required in front of \texttt{ergoimport}
...
The first form for {\tt '\bs{}\bs{}ergoimport'} above is used to both import
...
In {\tt '\bs{}\bs{}ergoimport'}, \emph{ergo-predicate} is the name of the imported
...
import an \FLSYSTEM non-tabled predicate {\tt \%foobar} of arity 3 one can use
the following statement:
%%
\begin{verbatim}
   ?- '\\ergoimport' '%'(foobar)/3 as foobar(_,_,_) from mymodule.
\end{verbatim}
%%
...
command (again,  {\tt \FLBOOTSTRAP} must be executed in advance): 
%%
...
\end{quote}
%%
The first command loads the file {\it ergo-file\/} into the given user
module and compiles it, if necessary. The second command loads the knowledge
base
into the default module {\tt main}. The last two commands compile the file
...
everything) is a Prolog program.  If the initial module is an \FLSYSTEM
...
\subsubsection{Submitting Arbitrary Queries to \FLSYSTEM}
...
module. Sometimes, it is useful to be able to submit arbitrary queries to
\FLSYSTEM. This is particularly useful when \FLSYSTEM runs under the
control of an external program written in C, Python, or Java.
...
%%
\begin{itemize}
  \item  \emph{String}: A string that contains an \FLSYSTEM query. It can be an
    atomic frame
...
i.e.}, {\tt abc}). These scoping rules have subtle effect on literals that
...
ends up being  {\tt (a[b->c]@foo)@bar}, i.e., {\tt a[b->c]@foo}.
...
\end{verbatim}
%%
In the second query, the variable {\tt ?N} gets bound to \texttt{foo}.

Operator precedence should also be kept in mind when dealing with
meta-unification. For instance,
%% 
\begin{verbatim}
   ?- ?X ~ (a[b->c],a[d->e]).
\end{verbatim}
%% 
yields \verb|?X = (${a[b->c]@main}, ${a[d->e]@main})| but
%% 
\begin{verbatim}
   ?X ~ a[b->c],a[d->e].
\end{verbatim}
%% 
fails because \verb|~| binds stronger than the \texttt{','} so the above
is read as   \verb|(?X ~ a[b->c]), a[d->e]|. This statement would usually return a
\texttt{No} unless \verb|a[d->e]|  is true.
...
\index{sleep\{...\}}
\paragraph{Waiting.}
It is possible to tell \FLSYSTEM to wait for a number of milliseconds as follows:
%% 
\begin{alltt}
 \prompt  sleep\{150\}.
\end{alltt}
...
\subsection{Passing Terms between \FLSYSTEM and Prolog}\label{sec-passing-args}
...
Unlike Prolog, which always returns the answers in the one-at-a-time
fashion, \FLSYSTEM computes the
answers for the entire clique of inter-dependent
subgoals before letting the computation to proceed to the next subgoal
in the rule body. The following small example illustrates the difference:
%%
\begin{verbatim}
a:b.
d:b.
...
\end{verbatim}
%%
will produce the same result where \texttt{a, b, c} and \texttt{done}
...
recursive UDFs cannot be defined---one should just exercise some care.
...
Like many things in \ERGO, the scope of any  UDF definition is limited to
the module into which the file containing that definition gets loaded or
added. However, because UDFs are implemented as program transformations
done at compile time, that scope further depends on when any particular UDF
gets compiled, which happens when the corresponding file gets loaded or
added to the module for the \emph{first} time. 
Therefore, a UDF  has an effect from
...
directive).
This scope can be extended to the files added to the module in question 
later provided those files have the appropriate \texttt{useudf} directives
(Section \ref{sec-udf-multifile}). 

The definition of a UDF may include references to other modules, which, in
turn, may have their own UDF definitions, possibly even with the same names.
It should be clear that the uses of the UDFs in those called modules are subject to the UDF
definitions that occur in the called modules, not in the caller modules.
For instance, consider
%%
\begin{alltt}
...
\texttt{func1}, that  other definition has no effect on the above occurrence of
\texttt{func1(?x)} in the above snippet.   Thus, the query
%%
\begin{alltt}
?- p(c).
\end{alltt}
...
\end{alltt}
%%
A UDF definition can reference multiple other modules as well. For instance,
%%
\begin{alltt}
    \bs{}udf foo(?x):=?y \bs{}if pred(?x,?y)@bar, pred(?x,?y,?z)@moo, q(?z).
\end{alltt}
%%
At present, UDF definitions that involve update operations or predicates and methods
that begin with a ``\%'' are not  supported. If a UDF definition
depends on an update operation then a runtime error will result. For instance, if a UDF
...
\section{Controlling Multi-Context Symbols} \label{sec:symbol}
...
The primitive \texttt{erasemodule} empties the named module out but does not delete
it from the registry of modules, so new facts and rules can be inserted
into that module again \emph{without the need to recreate} the module with
...
possibly, other meta-data) and the property that such queries are not
...
\end{alltt}
%% 
The descriptor part is mandatory and has the same form as the rule
descriptors introduced in Section~\ref{sec-rule-id}.  The \emph{query-body}
part is the same as the body of a query. Thus, from regular queries the
latent queries differ in that they have descriptors and they use
\texttt{!-} instead of \texttt{?-}. From rules, this new type of queries
differs in that they have no heads and use \texttt{!-} instead of
\texttt{:-}.  So, even though latent queries have rule descriptors, the
absence of the rule heads makes the concept of defeasibility inapplicable
and likely even meaningless for such queries.\footnote{ However, as
  mentioned, latent queries can be disabled and then re-enabled.}
Defeasibility is discussed in Section~\ref{sec-defeasible}.
...
because the interpretation of \texttt{\bs{}@F} depends on the file name where
this quasi-constants appears and \texttt{\bs{}@} is
interpreted with respect to the module in which that quasi-constants
...
\index{conflict among rules}
\index{opposition among rules}
...
\index{GCL!argumentation theory}
\index{argumentation theory!GCL}
\index{\bs{}gcl module}
...
\index{primitive!tag\{...\}}
%%
Sometimes it is useful to be able to query rulebases using the rule
tags and rule heads as search keys. Such queries can be issued
...
\textbf{Note}: only rules and facts can be  defeasible because this concept  
is not meaningful for UDFs, queries, and latent queries, even though some of
these may have descriptors for other uses.
These constructs have no analogue for rule heads, which
are necessary in order to even just define what a conflict is.
...
\index{GCL!cautious argumentation theory}
\index{argumentation theory!GCL}
\index{OGCL!original GCLP argumentation theory}
\index{argumentation theory!OGCL}
...
i.e., there can be no sequence of
  ...
  \index{RefuteCLP!rebuttal-less argumentation theory}
  \index{argumentation theory!RefuteCLP}
...
\end{verbatim}
  ...
  %%Generally, \texttt{!=} and \texttt{!==} are slower but safer and more declarative
...
\item \texttt{\bs{}callable} --- this class includes all atomic formulas
  ...
  former is false because \texttt{p(b,?X)} is a HiLog term, not a predicate,
...
\noindent
\index{metaclass\{...\} primitive}
\index{primitive!metaclass\{...\}}
\textbf{metaclass\{...\} primitive.}   
Related to all this is the \texttt{metaclass\{?x\}} primitive, which
returns all the classes that are specified as builtin by \ERGOAI.
Thus, given the database
%% 
\begin{verbatim}
  a:bbb.
  c:ddd.
\end{verbatim}
%% 
then the query ?X:?Y returns 7 answers, some of which involving builtin classes 
...
7 solution(s) in 0.000 seconds; elapsed time = 0.000
\end{verbatim}
%% 
while this query returns only two answers, ignoring the builtins. In many cases,
the user might prefer the second variant.
%% 
\begin{verbatim}
ergo> ?X:?Y, \+ metaclass{?Y}.
?X = a
?Y = bbb
...
cardinality constraints only for the default values of the methods.
%% 
\begin{itemize}
\item
  {\tt
...
\index{timer interrupt handler!fail}
  ...
  \item A predefined error handler \texttt{pause}, 
    in which case, the execution pauses and the user is
    given an
    opportunity to inspect the state of the system and then either continue
    or abort the computation.
  The \texttt{pause} interrupt handler is  most useful when the
...
\item \texttt{Method[mustDefine(?Mode)]} --- affects the system behavior when
  ...
  User-initiated aborts can be caught as follows:
  %%
  \begin{alltt}
...
%% 
\noindent
\textbf{$\bullet$~~~ \tt  read(?Code,?Stat)@\bs{}parse.} ~~\\
...
succeeds. If called repeatedly, it will read the second, third, etc.,
  term from the input stream. On reaching the end of the file,
  \texttt{?Stat} will be bound to \texttt{[null,eof]}.  
%% 
\textbf{$\bullet$~~~ \tt  ?Stream[read(?Code,?Stat)]@\bs{}parse.} ~~\\
...
%% 
\noindent
\textbf{$\bullet$~~~ \tt  readOneByOne(?Code,?Stat)@\bs{}parse.} ~~\\
{$\bullet$~~~ \tt  readOneByOne(?Module)(?Code,?Stat)@\bs{}parse.} ~~\\
Used for reading terms one-by-one and returning answers interactively.
The meaning of the arguments is the same.  In the one-at-a-time mode,
the {\tt \bs{}one}command, will wait for the input, return the compiled code, then wait for
more input, if the user types ";". If the user types {\tt RET} then this
predicate succeeds and exits. If the user types \texttt{;<RETURN>}, the next answer is printed and the system will wait for more input again.
   In the all-solutions mode ({\tt
  \bs{}all}), will wait for more input to process, but will not return
...
\noindent
\textbf{$\bullet$~~~\tt  ?Stream[readOneByOne(?Code,?Stat)]@\bs{}parse.} ~~\\
...
\noindent
\textbf{$\bullet$~~~\tt  ?Source[readAll(?CodeList)]@\bs{}parse.}~~\\
...
Another common mistake is misuse of complex frames in
...
\end{verbatim}
%%
Forgetting this tautology may sometimes led to a logical error.
...
\end{verbatim}
%%
is likely a mistake if the user simply wanted to convert the
relational representation into the frame representation. Indeed,
here the head
is a \emph{single} object with an Id \texttt{flightobj} and therefore both {\tt from} and
{\tt to} will normally be multi-values, so it will not be possible to find out
(by querying that object) which cities have direct flights between them.
The easiest way to see this is to realize that the above is equivalent to
%%
\begin{verbatim}
...
\end{verbatim}
%%
then the following frames will be derivable (where the last two are
unintended):
%%
\begin{verbatim}
...
\end{verbatim}
%% 
in each file. However, oftentimes this is inconvenient, time-consuming, and might be
...
specification
might still be not executable on its own. For instance,
it might contain calls to
...
\item  \emph{Embedded ISA-literals in rule heads}.
   ...
   are not advisable to novice users is because a novice tends to confuse
   such literals
...
\item  \emph{Expanded scope for the operators $\mvd$ and $\Mvd$.}
...
\section{Enabling and Disabling of  Advanced Features}\label{sec=disablefeature}

In other sections  of this manual we described the various advanced
features of \ERGO, which make Ergo more logical and 
useful. At the same time, the added usability may affect the performance
and one may prefer to turn some or all of these features off.
These features are:
%% 
\begin{itemize}
\item \texttt{naf\_delay} --  disabling this makes \texttt{\RULELOGNAF}
  execute immediately, without delaying --- even for nonground subgoals. 
  This makes \texttt{\RULELOGNAF} faster, but not purely  logical.
\item \texttt{defeat\_delay} -- disabling this makes testing for whether
  the rules are  defeated immediate, even for non-ground such tests.
  This makes defeasible reasoning faster, but ``less correct''
  in some edge cases.
\item \texttt{subgoal\_delay}: support for the delay quantifiers
  \texttt{must} and \texttt{wish} as well as the delayable builtins like
  \texttt{isinteger/2}, \texttt{isvar/2}, and \texttt{!=}.
\item \texttt{attr\_vars} (attributed variables): enables the use of
  \emph{attributed  variables}  to delay computation. Disabling this also
  disables the above three features and some others, like CLPR-style
  constraint solving introduced in Section~\ref{sec-clp}.
\end{itemize}
%%
All these features are enabled by default.
Disabling \texttt{attr\_vars} also automatically disables 
\texttt{naf\_delay}, \texttt{defeat\_delay}, and \texttt{subgoal\_delay}.
Moreover, if one needs to enable any  of the above three ``delay'' features,
\texttt{attr\_vars} must be enabled first.
The primitives for turning the above features on or off are:
%% 
\begin{itemize}
\item  \texttt{enablefeature\{} \emph{feature} \texttt{\}}   
\item  \texttt{disablefeature\{} \emph{feature} \texttt{\}}   
\item \texttt{featureenabled\{}  \emph{feature-or-var} \texttt{\}}  
\end{itemize}
%% 
Where \emph{feature}   is one of the above listed terms.
%% 
For example,
%%
\begin{verbatim}
 ....  :-  ....,                       // \naf is delayed here
           disablefeature{naf_delay},
           ....                        // \naf is not delayed here
           enablefeature{naf_delay},
           .....                      // here \naf is delayed again
...
First, whenever you edit an \FLSYSTEM file, you will see the ``\emph{\floraname}'' menu
...
\end{alltt}
%%
When you invoke any of the above commands, an \FLSYSTEM process is started,
