...
\ERGOLITE
(also known as \FLSYSTEM)
is a sophisticated object-based  knowledge representation and
reasoning system. \ERGOLITE is an open source subset of its commercial
cousin called \emph{\ERGO Reasoner},\footnote{
  \url{http://coherentknowledge.com}
}
...
\index{FLORID}
%%
The language of \ERGOLITE/\FLSYSTEM is a dialect of \fl with
...
\ERGOLITE/\FLSYSTEM is available at \url{http://flora.sourceforge.net}.
This manual will mostly refer to the system under its traditional name,
\FLSYSTEM.
}
{\input{./extensions/intro}}
...
The Unix-style download is the file \texttt{flora2.run}. 
...
\end{quote}
%% 
This will create the folder \texttt{Flora-2} with
two subfolders: \texttt{flora2} and \texttt{XSB}.
The script to run \FLSYSTEM is \texttt{\textbf{DIR}/Flora-2/\ENGINENAMEALT/\ENGINERUN}.  

\paragraph{Windows.}
The Windows installer is called \texttt{flora2.exe}.
...
The command to run \FLSYSTEM is \texttt{./\ENGINERUN} (Linux/Mac/BSD/etc.)
...
Compile XSB, as explained in XSB's manual, Section 2.2.2.
...
\sloppy
Next, check out \FLSYSTEM from its SVN repository\footnote{
  To download, use:
\texttt{svn~checkout~svn://svn.code.sf.net/p/flora/src/trunk flora-src}
}
%%
...
\begin{quote}
  \tt
   ...
   makeflora clean\\
   \tt
   makeflora \textnormal{\emph{path-to-}}{\bs}XSB{\bs}bin{\bs}xsb64.bat
\end{quote}
%%
...
%%
\begin{alltt}
 alias \ENGINENAME='~/ENGINEDIR/\ENGINENAMEALT/\ENGINERUN'
\end{alltt}
%%
...
\begin{alltt}
 ...
 ?X = '/home/foo/test'
 // auxdir: hidden files dir; may be needed for some installations (e.g. Tomcat)
 \prompt system\{auxdir=?X\}.
 ?X = '/home/foo/.xsb/flora2-1.3-ad77352'
\end{alltt}
%% 
...
\item Proprietary \FLSYSTEM-specific extensions include arbitrary
  first-order formulas in the rule head, scalability
  improvements, OWL/SPARQL connectivity, JSON interface, the integrated
  development environment (Studio), and more.
}{}
\end{enumerate}
%%
}
...
disjunction is made using the infix operator ``{\tt ;}''.  Negation is
specified
using the prefix operators ``\PLGNAF'' and ``\RULELOGNAF''.\footnote{
  %%
...
\emph{Global Skolems} are  always numbered or named (no \verb|\##|),
and the scope of
...
%%
\begin{alltt}
 ?- add_lib_dir(a('/home/me/\ENGINENAMEALT')).  
\end{alltt}
%%
...
%%
\begin{alltt}
 ?- add_lib_dir(a('/home/myHomeDir/\ENGINENAMEALT')).  
\end{alltt}
%%
  \item The query
    %%
...
\ifdef{\isfloraman}{
However, {\tt \RULELOGNEG p,q} is not allowed in the
rule head, since this is tantamount to a disjunction, {\tt \RULELOGNEG p \bs{}or \RULELOGNEG q}.  
}
{}
...
\ifdef{\isfloraman}{}
{
  In \ERGO, \texttt{p(?Y) \rnegarr q(?Y)} may also occur in rule heads,
...
checked and \ERGO has also means of automatic enforcement of such
constraints.
}
{
Section~\ref{sec-typechecking} explains how type constraints can be
...
The \texttt{once\{UnitaryGoal\}} primitive
will always return the same answer
even if  \texttt{UnitaryGoal} has multiple answers.
\ifdef{\isfloraman}{
  \ERGO also has a \texttt{random\{UnitaryGoal\}} primitive, which would
  return randomly chosen answers (each time different) instead.
}
{
\index{random\{...\} primitive}
\index{primitive random\{...\}}
\index{random answers}
  The \texttt{random\{UnitaryGoal\}}
  primitive will instead return randomly chosen answers of \texttt{UnitaryGoal}. 
...
\emph{are} allowed in UDF definitions (in
$\ERGO$ only).
Example:
%% 
{\tt
...
At the same time, latent queries is an older mechanism, which is primarily
retained for \FLORA and \ERGOLITE users. \ERGO has newer, much more
...
\ifdef{\isfloraman}{
  In \ERGO, latent queries are used for automatic maintenance of integrity
  constraints---a feature not available in \FLORA/\ERGOLITE.
}
{
...
%%
\ifdef{\isfloraman}{
  Note: the methods \texttt{date} and \texttt{time} listed above exist only in \ERGO.
}
...
%%
\ifdef{\isfloraman}{
 Note: the methods \texttt{toDate}, \texttt{minus} and \texttt{weekday}
 through \texttt{weekend} exist only in \ERGO.
}
...
%% 
\ifdef{\isfloraman}{
  Note: the examples involving \texttt{minus} and \texttt{weekday} through
  \texttt{weekend} 
  will not work in \FLORA ---
  only in \ERGO.
}
...
%%
\ifdef{\isfloraman}{
 Note: the methods \texttt{toDateTime}, \texttt{minus} and \texttt{weekday}
 through \texttt{weekend} exist only in \ERGO.
}
...
\begin{itemize}
  ...
  \item \texttt{?- "2011-12-22"$\hat{~}\hat{~}$\bs{}date[toDateTime(11,15,7.6) ->}
    ...
    \ifdef{\isfloraman}{
    \\
    This example will not work in \FLORA --- only in \ERGO.
    \\
    }
...
%% 
\ifdef{\isfloraman}{
  Note: the examples involving \texttt{toDateTime},
  \texttt{minus} and \texttt{weekday} through
  \texttt{weekend} 
  will not work in \FLORA ---
  only in \ERGO.
}
...
%%
\ifdef{\isfloraman}{
  Note: the  methods \texttt{minus} and \texttt{toDateTime} exist only in \ERGO.
}
...
\item \texttt{prolog=[Opt1,Opt2,...,OptN]} -- pass the list of Prolog
  ...
  \ifdef{\isfloraman}{}
  {
  \item In \ERGO, there is also the option \texttt{omni=on} (and
...
\item
  ...
  \ifdef{\isfloraman}{\texttt{flora\_read}}{\texttt{ergo\_read}}
...
\ifdef{\isfloraman}{\texttt{flora\_read}}{\texttt{ergo\_read}}
(and
\ifdef{\isfloraman}{\texttt{flora\_read(?Module)}}{\texttt{ergo\_read(?Module)}})
...
special major mode, \emph{\FLORAMODE}.
...
\subsection{Installation of \FLORAMODE}


To install \emph{\FLORAMODE}, you must perform the following steps. Put the
file
%%
\begin{quote}
  {\tt .../\ENGINENAMEALT/emacs/\FLORAFILE}
\end{quote}
%%
...
\texttt{\FLORAFILE}  there. Such a directory can be added to the emacs search path by
...
Emacs will be able to invoke \texttt{\FLORAMODE} automatically when you
are editing such files:
%%
\begin{alltt}
\small
(setq auto-mode-alist
      (cons \textquotesingle(\ergoextemacs . \FLORAMODE) auto-mode-alist))
(autoload \textquotesingle{}\FLORAMODE "\FLORAFILEBASE" "Major mode for editing \FLSYSTEM knowledge bases." t)
\end{alltt}
%%
...
\subsection{Functionality of \FLORAMODE}
...
In addition to the menu, \emph{\FLORAMODE} lets you execute most of the
