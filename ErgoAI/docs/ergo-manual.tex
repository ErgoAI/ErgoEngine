...
The easiest way to install \ERGOAI is to download an official release,
...
like the OWL/RDF import. (This includes the requirement that the
environment variable \texttt{JAVA\_HOME} must point to the correct JDK.)
Some functions in the studio will not work with
...
\subsection{Installing \ERGOAI with an Officially Released Installer}\label{sec-inst-prereq}
...
\item[] \textbf{Windows:} 
  %% 
  \begin{itemize}
  \item Download the file \texttt{ergoAI\_\textnormal{\emph{versn}}.exe}  from Github, where
    \emph{versn} is the version number. 
  \item Close any other running copy of \ERGOAI.
  \item Double click on \texttt{ergoAI\_\textnormal{\emph{versn}}.exe}
    to open a setup
...
See Section~\ref{sec-inst-prereq} for the details on prerequisites.
...
\sloppy
Next, clone \FLSYSTEM from its repository on Github\footnote{
\texttt{???}
}
%%
into a
...
for the \FLSYSTEM engine in your git repository; it should have the form
...
\end{verbatim}
%% 
To create such a link, use \texttt{ln -s} on Linux/Mac and   
\texttt{mklink /j} on Windows:
%% 
\begin{verbatim}
