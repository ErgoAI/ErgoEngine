...
#Usage: testsuite_new.sh [-opts opts] [-tag tag] [-exclude exclude_dirs] \
...
if test $# -gt 1; then
    echo "Usage: testsuite_new.sh [-opts opts] [-tag tag] [-exclude \"excl_dirs\"] [-add \"add_dirs\"] [-mswin] path"
...
/usr/bin/time \
./testall.sh -opts "$options" -exclude "aux_files cyc150/core_files $excluded_tests" \
                              -add "$added_tests"  \
